...
```{r}
#p(lambda)
prior <- c(0:99) # 100 values
plot(prior, type="h", main = "Prior", lwd = 2, col = "darkblue")
```
...
So a discrete uniform probability distribution (```posterior```) is calculated by repeating each probability.


```{r}
posterior <- rep(1/length(prior),times = length(prior)) 
#plot the discrete uniform probability distribution
plot(posterior, type="h", main = "Posterior", lwd = 2, col = "darkblue")
```
...
plot(likeli,typ="h", main = "Likelihood", lwd = 2, col = "darkblue")
```
...
```{r}
bayes_rate <- likeli * posterior
bayes_rate <- bayes_rate/ sum(bayes_rate)
plot(bayes_rate, type = "h", main = "Bayes Rule", lwd = 2, col = "darkblue") 
```

The shape of the distribution is exactly the same (see graphic likelihood) when a flat probability is used. 
The only difference is that the probabilities in the *likelhood diagram* are much smaller. Similar to the poisson distribution, the probabilities of the bayes rule are also considered independently of each other. This implies that neither an equal distribution nor a conditional probability curve is generated.
