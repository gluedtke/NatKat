...
We are going to use the **Bayes' Rule** in order to solve this problem. The Bayes Rule is defined as follows: $$p(\kappa|D)=\frac{p(D|\kappa) p(\kappa)}{p(D)}$$ where D are the observed data and $\kappa$ are the things that we can learn from observed data.
...
The following approach was adopted from [Mages Blog](https://magesblog.com/post/2014-07-29-hit-and-run-think-bayes/).
...
```{r}
#p(k) = p(wildfire caused by lightning)
prior <- 1/1000
...
Now we are looking for the probability that the eyewitness is right and actually observed a wildfire that was caused by a lightning.
...
In this case, we can use the law of total probability.

$$p(D) = p(D|\kappa)p(\kappa)+p(D|-\kappa)p(-\kappa)$$

where ```-``` indicates the complementary event.

In R we can write this as:

```{r}
#p(D) = p(witness said the wildfire was caused by lightning)
...
```{r}
#p(k|D) = p(wildfire caused by lightning|witness said the wildfire was caused by lightning)
post <- likeli * prior / evid
```
...
```{r, echo = FALSE}
post_pro <- round((post * 100), digits = 1) 
```

Thus, even if eyewitness reports are reliable 80 % of the time, the probability of the witness being true about a lightning-caused wildfire is **``r post_pro`` %**.
