...
By using the likelihood, the prior is updated into the **posterior**. The posterior is a probability distribution of the values specified in the prior belief. It shows how probable a particular parameter value is when the data are given. Thus, the posterior can be used in order to determine what we can learn about a parameter of the dataset. The only difference between the prior and the posterior is that the posterior has seeing the data (Stone 2013: 79).

## **3. Review of the Application of Bayes' Rule in Hazard Assessment**

In order to introduce the role of the Bayes' Rule, 20 studies, dealing with its application for different natural hazards, have been prepared. Moreover, this review helps to get a better understanding of the Bayesian parameters (prior, likelihood) in the context of the investigation of natural hazards.
...
Elsner and Jagger (2004) use an (hierarchical) Bayesian approach in the climatological analysis of hurricanes in the U.S with less accurate historical records. The hurricane frequency, in terms of annual hurricane counts conditioned on the state of El Nino Southern Oscillation (ENSO) and North Atlantic Oscillation (NAO), is modelled. Thus, the influence of these climate factors is examined. The comparison of the posterior distributions from Bayesian models using noninformative and bootstrap priors shows, that the influence of ENSO factors on hurricane activity decreases while the effect of NAO factors enhances slightly when using a bootstrap prior.

***

*A Quantitative Model for Volcanic Hazard Assessment*:
In order to quantify the long-term volcanic hazard, Marzocchi et al. (2006) define an event tree scheme for an explosive volcano in order to evaluate the probability for several possible volcanic events. Bayesian methods are implemented in the event tree scheme. Bayes' Rule is used to estimate the probability of events at each node of the event tree. For a hypothetical volcano, they determined a probability of 0.036 that volcanic unrest occur and an eruption as well as magmatic intrusion are detected.

***

*Probability of Tornado Occurrence across Canada*:
Cheng et al. (2013) estimate the probability of the occurrence of tornados in Canada by using a Bayesian approach. This approach is appropriate since the documented number of tornado occurrences is often biased and underreported due to varying population densities. The Bayesian modelling approach allows including bias in tornado observations. Analyses show, that the actual (modelled) number of tornado occurrences is at least twice the number of recorded tornados (150 and 70 $yr^{-1}$, respectively). 
...
Meinshausen et al. (2009) aim for quantify the greenhouse gas emissions for the period 2000 to 2050 that account for a global warming limit of 2 °C. They choose a Bayesian approach for modelling the climate sensitivity. Results show that the probability of exceeding 2 °C can be limited to below 25 % by $CO_2$ emission below 1000 Gt.
...
*Note: The values in this and the following examples are fictitious and not based on scientific investigations.*
...
We assume that Irma has no information about which of the cyclone rates is more probable. Thus, a discrete **uniform probability distribution** is used to give the same weight to all these possible outcomes. This results in equally probable cyclone rates. The prior probability (Fig. 2), described before, is easily calculated by assigning each outcome the reciprocal of the total number of outcomes.
...
In the following step, the likelihood (probability of the observed annual cyclone counts given the prior-defined expectation rates) is calculated. As mentioned above, we use a Poisson distribution as best description for our cyclone problem. Since all cyclone events are independent of each other, the likelihood can be calculated by multiplying several Poisson distributions. For each distribution, Irma's dataset is the fixed input variable for which the probability should be calculated. However, the used event rate $\lambda$ differs. Here, each value of the prior assumption about the expectation rate, specified above, is used. The resulting likelihood is shown in Fig. 3.
...
*During a field trip and by using remote sensing, geologist Thomas identified past landslides for a mountain range in China and estimated the area of each individual landslide. For scientific reasons, he is now interested in how the landslide areas are distributed in order determine which range of minimum areas are most probable to result from future landslides. * 
...
We choose a **Pareto distribution** in order to learn something about the size distribution of landslide areas, as shown by Stark (2002). A Pareto distribution can be applied to problems where a variable follows a power-law with a small range of values that have a comparable high probability. The distribution can be described by the shape parameter (or Pareto Index) $\alpha$ and the scale parameter $\theta$. The shape parameter defines the ratio of the probability of smaller values to that of larger values - the larger $\alpha$ the smaller is the proportion of very high values. The scale parameter is the minimum value of the variable under consideration. It defines also the maximum of the Pareto distribution (Tiwari & Zalkikar, 1990). 

According to Stark (2002), values for the landslide area come from a Pareto distribution. You can think about this distribution as a model to describe landslide areas. $\theta$ is already fixed due to our data (further described below). However, we are uncertain about $\alpha$. Therefore, we have to specify the prior assumption about $\alpha$ in order to quantify its uncertainties. So we can work with the landslide model and help Thomas.
...
For our prior belief, we have to specify the shape parameter (Pareto Index) $\alpha$, that is unknown for our landslide area distribution. That means that we specify to what extent lower landslide areas are more probable than higher landslide areas. 
...
Moreover, we know that lots of landslide data show a Pareto distribution of $\alpha = 3/2$, as reported by Stark (2002). We also found out that  in general lower and higher Pareto Indices are less probable. The probability decreases also with increasing distance from $\alpha = 3/2$. In order to account for this knowledge, a **normal distribution** is used for weighting the specified Pareto Indices. The mean of the distribution $\mu$ is set to 1.5 since this Pareto Index should have the highest probability. Furthermore, the standard deviation $\sigma=0.2$ is chosen in order to create a curve that is close to the mean. Since we create with `dnorm` a probability density for the normal distribution, we have to renormalize the result to obtain the prior probability distribution for the specified Pareto Indices. Figure 7 shows the prior probability. 
...
```{r, fig.cap = "Figure 7: Prior probability distribution of the Pareto Index.", fig.align="center", echo=FALSE}
...
The resulting estimated posterior probability for the Pareto Index of Thomas' landslide data is shown in Figure 9. A Pareto Index of 1.39 is most probable to describe the distribution of Thomas' landslide area data. Higher and lower indices around this value have a similar probability (see Fig. 9).
That's why, we can now create a Pareto distribution with the landslide area as input variable and a shape parameter of 1.39. Figure 10 indicates, that lower landslide areas are more probable than higher landslide areas. For the mountain range investigated by Thomas, the occurrence of landslides smaller than 100 $m^2$ is more likely than the occurrence of landslides with greater areas.

```{r, fig.cap = "Figure 10: Pareto probability distribution of landslide areas.", fig.align="center", echo=FALSE}
...
This report dealt with the application of the Bayes' Rule in connection with natural hazards, risks and disasters.

The first section showed that the Bayes' Rule is a good method to learn about data
and to quantify uncertainties associated with the data by determining probability distributions. The Bayes' Rule aims at calculating the posterior probability distribution. In order to calculate the posterior, at least the prior and the likelihood have to be defined.

The Bayesian approach is the focus of numerous studies in which specific natural hazards are analysed and predicted. In this report, 20 of these studies were analysed with regard to the use of Bayes' Rule to learn from obtained data and its parameters. These studies demonstrated that applying Bayes' Rule for hazard analysis is advantageous due to the ability of taken uncertainties into account. Moreover, the various possibilities of applying Bayes' Rule to different natural processes and kinds of parameters (e.g., size, temporal, spatial) were presented.

Subsequently, self-selected examples of applying Bayes' Rule to natural hazards, risks and disasters were created. These served once again for better comprehensibility of the Bayesian approach. Together with the reviewed studies, also the strengths and weaknesses of the Bayes' Rule could be identified.

One critical point is that numerous parameters can cause errors (e.g., calculation errors) that could continue in the following calculation steps. Moreover, if our problems are more advanced and complex, the successive computing steps can require a certain amount of complexity. In addition, the different steps of computation could be questioned with regard to the transparency.

On the other hand, Bayes' Rule  enables us to learn interesting things given very simple information. In this context, the most important advantages of a Bayesian approach is that uncertainties for parameters are taken into account. As the prediction of specific natural hazards is a sensitive issue, it is necessary not only to determine the mean or maximum of a parameter. Thinking about the distribution of parameter values is much more important. You can also easily update probabilities by using the Bayesian approach in the case of acquiring new knowledge. Furthermore, the implementation of Bayes' Rule in R showed that we can analyse different, rather simple, problems by using a similar code. This highlights the simplicity of the realization of the Bayes' Rule.
...
BERTI, M. et al. 2012. *Probabilistic rainfall thresholds for landslide occurrence using a Bayesian approach*. Journal of Geophysical Research: Earth Surface, 117. Nr. F4.

BRINKERHOFF, Douglas J. 2016. *Inversion of a glacier hydrology model*. Annals of Glaciology 57. S. 84-95
...
CHENG, Vincent YS, et al. 2013. *Probability of tornado occurrence across Canada*. Journal of Climate, 26. Nr. 23, S. 9415-9428.

COLES, Stuart; TAWN, Jonathan., 2005. *Bayesian modelling of extreme surges on the UK east coast*. Philosophical Transactions of the Royal Society of London A: Mathematical, Physical and Engineering Sciences, 363. Nr. 1831, S. 1387-1406.

ELSNER, James B., JAGGER, Thomas H., 2004. *A  hierarchical Bayesian approach to seasonal hurricane modeling*. Journal of Climate, 17. Nr. 14, S. 2813-2827.

FERRAES, Sergio G., 1985. *The Bayesian probabilistic prediction of strong earthquakes in the Hellenic arc*. Tectonophysics, 111. Nr. 3-4, S. 339-354.
...
HO, Chih-Hsiang, SMITH, Eugene I., 1997. *Volcanic hazard assessment incorporating expert knowledge: application to the Yucca Mountain region, Nevada, USA*. Mathematical geology, 29. Nr. 5, S. 615-627.

International Journal of Climatology, 2013. *Bayesian approach to detecting change points in climatic records*. 33. Nr. 2, S. 520-528.
...
KWON, Hyun-Han,  BROWN, Casey & LALL,  Upmanu, 2008. *Climate informed flood frequency analysis and prediction in Montana using hierarchical Bayesian modeling*. Geophysical Research Letters 35. Nr. 5. DOI: 10.1029/2007GL032220 

LOPEZ, Ana, et al. 2006. *Two approaches to quantifying uncertainty in global temperature changes*. Journal of Climate, 19. Nr. 19, S. 4785-4796.
...
MARZOCCHI, Warner.; SANDRI, L.; FURLAN, C., 2006. *A quantitative model for volcanic hazard assessment*.

MARZOCCHI, Warner et al. 2004. *Quantifying probabilities of volcanic events: The example of volcanic hazard at Mount Vesuvius*. Geophysical Research Letters 109. Nr. 11. DOI: 10.1029/2004JB003155

MEINSHAUSEN, Malte, et al., 2009. *Greenhouse-gas emission targets for limiting global warming to 2 °C*. Nature, 458. Nr. 7242, S. 1158.

MIN, Seung-Ki; HENSE, Andreas; KWON, Won-Tae, 2005.* Regional-scale climate change detection using a Bayesian decision method*. Geophysical research letters, 32. Nr. 3.
...
PERUGGIA, Mario; SANTNER, Thomas., 1996. *Bayesian analysis of time evolution of earthquakes*. Journal of the American Statistical Association, 91. Nr. 435, S. 1209-1218.
...
ROSSANO, S., et al., 1996. *Computer simulation of pyroclastic flow movement: an inverse approach*. Geophysical research letters, 23. Nr. 25, S. 3779-3782.

RUGGIERI, Eric 2013. *Short Communication A Bayesian approach to detecting change points in climatic records*. International Journal Of Climatology 33. S. 520-528
...
STRAUB, Daniel & GRÊT-REGAMEY, Adrienne, 2006. *A Bayesian probabilistic framework for avalanche modelling based on observations*. Cold Regions Science and Technology 46. S. 192-203

TIWARI, Ram C., ZALKIKAR, Jyoti N., 1990. *Empirical Bayes estimation of the scale parameter in a Pareto distribution*. Computational Statistics & Data Analysis 10. Nr. 3, S. 261-270.
...
WERNER, Angelika; HOLBROOK, Neil J., 2011. A. *Bayesian forecast model of Australian region tropical cyclone formation*. Journal of Climate,  24. Nr. 23, S. 6114-6131.

YADAV, R.,B.,S. Et al. 2013. *An evaluation of tsunami hazard using Bayesian approach in the Indian Ocean*. Tectonophysics.  593. S. 172-182
