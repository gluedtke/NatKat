...
In the next step, ```n``` random data points are created from this Gaussian distribution with the mean $\mu$ (= 5.2) and variance $\sigma$ (= 0.1) by using ```rnorm()```. For this function, the standard deviation $\sigma$ instead of the variance $\sigma^2$ must be used. 
...
Finally, we multiply prior and likelihood and re normalize in order to calculate the posterior. 
...
abline(v = 5.2, lty = 2, lwd = 2)
#legend(3.5, 0.4, legend=c("", "given mean"),
      # col=c("black", "black"), lty=0:2, cex=0.8)
...
Results show that the calculated most believable mean is close to / equal to the given mean.
